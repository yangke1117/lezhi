 ...
 $AUTH = new \Think\Auth(); if($AUTH->check('Admin/Rule/index', session('admin.id'))){ ?>
	...
	<div class="main-content">
		<div class="breadcrumbs" id="breadcrumbs">
			...
			<div class="nav-search" id="nav-search" method="get">
				<form class="form-search" action="/lezhi/index.php/Admin/Goods/index" method="get">
					<span class="input-icon">
						<select name="num" >
				            <option value="10" <?php if($num == 10): ?>selected<?php endif; ?> >10</option>
				            <option value="25" <?php if($num == 25): ?>selected<?php endif; ?>>25</option>
				            <option value="50" <?php if($num == 50): ?>selected<?php endif; ?>>50</option>
				            <option value="100" <?php if($num == 100): ?>selected<?php endif; ?>>100</option>
				        </select> 
					</span>
					<span class="input-icon">
						<input type="text" placeholder="Search ..." name="keyword" value="<?php echo ($keyword); ?>" class="nav-search-input" id="nav-search-input" autocomplete="off" />
						<input type="submit" class="btn btn-primary btn-xs" name="search" value="搜索" style="border-radius:3px; height:28px; margin-top:-4px; padding-right:24px;">
					...
					<div class="row">
						<div class="col-xs-12">
							<div class="table-responsive">
								<form action="/lezhi/index.php/Admin/Goods/del" method="post" >
								<table id="sample-table-1" class="table table-striped table-bordered table-hover" style="text-align:center;">
									<thead>
										<tr>
											<th class="center">ID</th>
											<th class="center">商品名称</th>
											<th class="center" class="hidden-480">分类名称</th>
											<th class="center" class="hidden-480">商品价格</th>
											<th class="center" class="hidden-480">图片</th>
											<th class="center" class="hidden-480">库存</th>
											<th class="center" class="hidden-480">状态</th>
											<th class="center" class="hidden-480">审核</th>
									...
									<tbody>
										<?php if(is_array($goods)): foreach($goods as $key=>$good): ?><tr>
											...
											<td class="hidden-480"><?php echo ($good[aname]); ?></td>
											<td><?php echo ($good[nowprice]); ?></td>
											<td><img src="/lezhi<?php echo ($good[goodpic]); ?>" width="50px"></td>
                                            <?php if(empty($goodsStock[$good['id']])): ?><td>已无库存</td>
                                                <?php else: ?>
                                                <td><?php echo ($goodsStock[$good['id']]); ?></td><?php endif; ?>


											<td><?php echo getState($good[state]);?></td>
											<td><?php echo getAudit($good[audit]);?></td>
											<td>
												<div class="center" class="visible-md visible-lg hidden-sm hidden-xs btn-group">

													<a href="<?php echo U('Admin/Goods/set',array('id'=>$good[id]));?>" title="设置详细信息" class="btn btn-xs btn-success">
														<i class="icon-ok bigger-120"></i>
													</a>
													<a href="<?php echo U('Admin/Goods/argument',array('id'=>$good[id]));?>" title="设置商品参数">
														<img src="/lezhi/Public/Admin/images/canshu.png" alt="" width="32.4063" height="26">
													</a>
													<a href="<?php echo U('Admin/Goods/edit',array('id'=>$good[id]));?>" title="修改商品状态" class="btn btn-xs btn-info">
														<i class="icon-edit bigger-120"></i>
													</a>

													<a href="<?php echo U('Admin/Goods/delete',array('id'=>$good[id]));?>" class="btn btn-xs btn-danger" onclick="if(!confirm('确定要删除吗，你确认要删除吗？')){return false;}">
														<i class="icon-trash bigger-120"></i>
													</a>
													
													<a href="<?php echo U('Admin/Goods/detail',array('id'=>$good['id']));?>" title="图片上传" class="btn btn-xs btn-warning">
										...
										<tr>
											<td colspan="9"><div class="row"><div class="col-md-5"></div><div id="pages"><?php echo ($pages); ?></div></td>
										</tr>
									</tbody>
								</table>
								</form>
