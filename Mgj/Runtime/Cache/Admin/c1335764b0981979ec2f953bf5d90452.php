					...
					<div class="row">
						<div class="col-xs-12">
							<div class="table-responsive">
								<form action="/lezhi/index.php/Admin/Group/update" method="post" enctype="multipart/form-data">
								<table id="sample-table-1" class="table table-striped table-bordered table-hover">
									<tbody>
										<tr>
											<td style="text-align:right;">用户组名：</td>
											<td><input type="text" name="title" id="form-field-1" placeholder="请设置用户组名" value="<?php echo ($res['title']); ?>" class="col-xs-10 col-sm-5" /></td>
										</tr>
										<tr>
											<td style="text-align:right;width:26%">可用权限：</td>
											<td>
											<?php if(is_array($rules)): foreach($rules as $key=>$v): ?><div style="width:200px;height:30px;line-height:30px;float:left">
												<input type="checkbox" style="margin-top:8px;width:24px" name="rules[]"  id="form-field-1" value="<?php echo ($v['id']); ?>" placeholder="规则 (控制器名/方法名)" class="col-xs-10 col-sm-5" <?php if(in_array($v['id'],$data)): ?>checked<?php endif; ?>/><?php echo ($v['title']); ?>
												</div><?php endforeach; endif; ?>
											</td>
										</tr>
										<tr>
											<td style="text-align:right;"><input type="submit" class="btn no-border" value="提交" style=""></td>
											<td>
