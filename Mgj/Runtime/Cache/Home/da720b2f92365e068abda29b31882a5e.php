...
<div class="col-xs-5" style="float:right;margin:50px 350px;">
	<h2>添加商品</h2>
	<form action="<?php echo U('Home/Sort/insert');?>" method="post" enctype="multipart/form-data">
		<input type="hidden" name="storeid" value="<?php echo ($_SESSION['home']['id']); ?>">
		<div class="form-group has-error">
			<label class="control-label" for="inputSuccess">商品分类名</label>
			<select class="form-control" name="asortid">
			<?php if(is_array($asorts)): foreach($asorts as $key=>$vo): ?><option value="<?php echo ($vo['asortid']); ?>" <?php if($id == $vo['asortid']): ?>selected<?php endif; ?>><?php echo ($vo['aname']); ?></option><?php endforeach; endif; ?>
			</select>
		</div>
		<div class="form-group has-error">
			...
			<select class="form-control" name="state">
				...
				<?php if(is_array($br)): foreach($br as $key=>$v): ?><option value="<?php echo ($v["bid"]); ?>"><?php echo ($v["name"]); ?></option><?php endforeach; endif; ?>
			</select>
		</div>
	   <div class="form-group has-error">
		  	<label class="control-label" for="inputError">商品名</label>
		  	<input type="text" class="form-control" id="inputError" name="goodname">
		</div>
		<div class="form-group has-error">
		...
		<div class="form-group has-error">
		  	...
		  	<input type="text" class="form-control" id="inputError" name="nowprice">
		</div>
		<div class="form-group has-error">
		...
		<div class="form-group has-error">
			<label class="control-label" for="inputError">商品描述</label>
    			<textarea name="describe" class="form-control" cols="75" rows="5"></textarea>
			</a>
		</div>
		<div class="form-group has-error">
