...
<div class="col-xs-5" style="float:right;margin:50px 350px;">
	<h2>修改商品</h2>
	<form action="<?php echo U('Home/Sort/goodupdate');?>" method="post" enctype="multipart/form-data">
		<input type="hidden" name="id" value="<?php echo ($good['id']); ?>">
		<div class="form-group has-error">
			<label class="control-label" for="inputSuccess">商品分类名</label>
			<select class="form-control" name="asortid">
			<?php if(is_array($asorts)): foreach($asorts as $key=>$vo): ?><option value="<?php echo ($vo['asortid']); ?>" <?php if($good['asortid'] == $vo['asortid']): ?>selected<?php endif; ?>><?php echo ($vo['aname']); ?></option><?php endforeach; endif; ?>
			</select>
		</div>
		<div class="form-group has-error">
			<label class="control-label" for="inputSuccess">分类说明</label>
			<select class="form-control" name="state">
				<option value="0">----------请----------选----------择----------</option>
				<?php if(is_array($br)): foreach($br as $key=>$v): ?><option value="<?php echo ($v["bid"]); ?>" <?php if($good['state'] == $v['bid']): ?>selected<?php endif; ?>><?php echo ($v["name"]); ?></option><?php endforeach; endif; ?>
			</select>
		</div>
	   <div class="form-group has-error">
		  	<label class="control-label" for="inputError">商品名</label>
		  	<input type="text" class="form-control" id="inputError" name="goodname" value="<?php echo ($good['goodname']); ?>">
		</div>
		<div class="form-group has-error">
		  	<label class="control-label" for="inputError">商品title</label>
		  	<input type="text" class="form-control" id="inputError" name="title" value="<?php echo ($good['title']); ?>">
		</div>
		<div class="form-group has-error">
		  	<label class="control-label" for="inputError">商品价钱</label>
		  	<input type="text" class="form-control" id="inputError" name="nowprice" value="<?php echo ($good['nowprice']); ?>">
		</div>
		<div class="form-group has-error">
			<a href="javascript:;" class="file">选择商品图片
    			<input type="file" name="goodpic">
    			<img src="<?php echo ($good['goodpic']); ?>" alt="" width="40">
			</a>
		</div>
		<div class="form-group has-error">
			<label class="control-label" for="inputError">商品描述</label>
    			<textarea name="describe" class="form-control" cols="75" rows="5"><?php echo ($good['describe']); ?></textarea>
			</a>
		</div>
		<div class="form-group has-error">
