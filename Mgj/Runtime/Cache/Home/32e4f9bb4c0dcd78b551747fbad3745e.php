        ...
        <div class="info_right">
            ...
            <div class="d_order_list">
                <div class="d_order_section">
                    <?php if(is_array($order)): foreach($order as $key=>$orders): ?><table class="d_order_table">
                            <tbody>
                            <!--订单列表头部开始-->
                                ...
                                <td colspan="7" style="padding-top:5px;">
                                    <div class="d_table_info">
                                            <span class="d_info_no">订单编号：
                                                <span class="d_order_num"><?php echo ($orders["ordernum"]); ?></span>
                                            </span>
                                        <span class="d_info_deal_time">成交时间：<?php echo (date( "Y-m-d H:i:s",$orders["ordertime"])); ?></span>
                                        <?php if(is_array($orders['mygoods'])): foreach($orders['mygoods'] as $key=>$stor): ?><a href="javascript:;"
                                               class="d_shop_name">店铺：<span><?php echo ($stor['shopname']); ?></span></a><?php endforeach; endif; ?>
                                    </div>
                            ...
                            <tr class="d_order_table_item">
                                <td class="d_goods" colspan="2">
                                    <?php if(is_array($orders['mygoods'])): foreach($orders['mygoods'] as $key=>$stor): if(is_array($stor['myattr'])): foreach($stor['myattr'] as $key=>$godatr): ?><a href="" class="d_pic" hidefocus="true" title="" target="_blank"><img
                                                    src="<?php echo ($stor['goodpic']); ?>" alt="" width="70"></a>

                                            <div class="d_order_desc">
                                                <div class="goods" style="margin-bottom:40px;">
                                                    <p>
                                                        <a href="<?php echo U('Home/Detail/index/', ['id'=>$stor['id']]);?>"
                                                           target="_blank"><?php echo ($stor['goodname']); ?></a>
                                                        <a href="javascript:;" target="_blank" class="d_order_snapshot">[交易快照]</a>
                                                    </p>

                                                    <p><?php echo ($godatr[0]); ?>
                                                        <span style="margin-left:170px;"><?php echo ($stor['price']); ?>.00</span>
                                                        </span>
                                                    </p>
                                                    <p><?php echo ($godatr[1]); ?>
                                ...
                                <td class="d_total" rowspan="1">
                                    <ul>
                                        <li>
                                            <p class="d_total_price">￥<?php echo ($orders['orderprice']); ?>.00</p>
                                ...
                                <td class="d_status" rowspan="1">
                                    <?php if($orders["isdelete"] == 1): ?><p class="d_order_p">关闭交易</p><?php endif; ?>
                                    <a class="d_order_link" href="<?php echo U('orderDetail',array('id'=>$orders['id']));?>"
                                       target="_blank">订单详情</a>
                                </td>
                                <td class="d_other" rowspan="1">
                                    <a class="d_order_link" onclick="del($(this))" href="<?php echo U('orderRecycle');?>"
                                       index="<?php echo ($orders['id']); ?>">删除订单</a>
                                </td>
                                <script>
                                    function del(obj)
                                    {
                            ...
                            <tr class="d_order_table_footer">
                                <td colspan="4">
                                    <ul>
                                        <li><p data-time="<?php echo ($orders['time']); ?>" class="d_order_icon_clock" style="color:#fc5382;"></p></li>
                                    </ul>
                                </td>
                                <td class="d_total">
                                    <span class="d_order_sub">总计：</span>
                                    <!-- ￥<?php echo ($order["orderprice"]); ?> -->
                                    ￥<?php echo ($orders['orderprice']); ?>.00
                                </td>
                                <?php if($orders["state"] == 0): ?><td class="d_status">
                                        <p class="d_wait_pay">等待付款</p>
                                    </td>
                                    <td class="d_other">
                                        <a href="javascript:;" class="d_pay_btn">付款</a>
                                    </td>
                                    <?php elseif($orders["state"] == 1): ?>
                                        <td class="d_status">
                                            <p class="d_wait_pay">等待收货</p>
                                        </td>
                                        <td class="d_other">
                                            <a href="<?php echo U('orderShou',array('id'=>$orders['id']));?>" class="d_pay_btn y_affirm">确认收货</a>
                                        </td>
                                    <?php elseif($orders["state"] == 2): ?>
                                        <td class="d_status">
                                            <p class="d_wait_pay">等待评价</p>
                                        </td>
                                        <td class="d_other">
                                            <a href="<?php echo U('orderComment',array('id'=>$orders['id']));?>" target="_blank" class="d_pay_btn">评价</a>
                                        </td>
                                    <?php elseif($orders["state"] == 3): ?>
                                        <td class="d_status">
                                            <p class="d_wait_pay">已评价</p>
                                        </td>
                                        <td class="d_other">
                                            <a href="<?php echo U('orderReview',array('id'=>$orders['id']));?>" target="_blank" class="d_pay_btn">追加评价</a>
                                        </td>
                                    <?php elseif($orders["state"] == 4): ?>
                                        <td class="d_other">
